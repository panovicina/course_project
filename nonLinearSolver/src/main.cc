...
struct Base {
    //! Количество видов
	ssize_t Species;
    //! Вектор первых моментов
    vec N;
    //! Матрица векторов значений вторых моментов - 1
    Eigen::Array<vec, Eigen::Dynamic, Eigen::Dynamic> D;
    //! Матрица векторов ядер конкурентной смертности
    Eigen::Array<vec, Eigen::Dynamic, Eigen::Dynamic> w;
    //! Матрица чисел коэффиуентов ядер конкурентной смертности
    mat dmat;
    //! Вектор векторов ядер рождаемости
    Eigen::Array<vec, Eigen::Dynamic, 1> m;
    //! Матрица значение \f$y\f$
    /*! Считается во время итераций. Используется для дебага */
    mat y;
    //! Вектор значений коэффициентов ядер рождаемости
    vec b;
    //! Вектор коэффициентов смертности от окружающей среды
    vec dvec;
    //! Вектор среднеквадратичного отклонения для ядер рождаемости. Используется только перед стартом решателя.
    vec sm;
    //! Матрица среднеквадратичного отклонения для ядер смертности. Используется только перед стартом решателя.
    mat sw;
    //! Значение расстояния между точками
    dl h;
    //! Размер области на котором считается модель
    dl A;
    //! Значение \f$\alpha\f$ для замыкания
    dl al;
    //! Количество точек на которых считается модель
    ssize_t points;
    //! Размерность на которой считается модель
    ssize_t dim;
    //! Количество итераций
    size_t iters;
